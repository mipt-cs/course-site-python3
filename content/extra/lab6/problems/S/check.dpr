...
var r:array [1..maxr,0..maxn] of longint;
	n,i,cc,ta,tb,tc, bestmoves:longint;
begin
 n:=inf.readlongint;
 r[1,0]:=n;
 ...
 while not ouf.seekeof do
  ...
  {if (tb=1) and (tc=maxr) then quit(_pe,format('Disk moving breaks the rules of the game at command #%d',[cc]));
  if (tc=1) and (tb=maxr) then quit(_pe,format('Disk moving breaks the rules of the game at command #%d',[cc]));}
 ...
 if cc > bestmoves then quit(_wa,'Not optimal solution'); 
 if cc < bestmoves then quit(_fail,'Solution is better then jury');
