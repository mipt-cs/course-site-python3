...
========================================
Объектно-ориентированое программирование - это методология программирования,
...
В общем случае, в разных языках программирования термин «инкапсуляция» относится к одной из или обеим одновременно следующим определениям:

    - механизм языка, позволяющий ограничить доступ одних компонентов программы к другим. Например, ограничивается доступ к переменным объекта класса. В Python, чтобы создать в классе *скрытую переменную*, такую переменную, что у ней имеют доступ только методы самого класса, нужно перед именем переменной поставить `__`. Давайте рассмотрим пример:
        ...
        1. Безопастность. Чтобы никто не мог залезть внутрь класса и не записать в переменные все что захочет и сломать вашу программу.
...
- *Снижение сложности*. Путем абстрагирования от внутренней реализации класса, мы сосредатачиваемся на интерфейсе, т.е на том, что может делать АТД, а не на том как это делается.
...
Класс Object определяет 10 методов, которые могут быть у объекта. И которые могут быть переопределенны.
...
Если забудем что-то поменять, то это приведет к ошибке. В масштабах большого программного продукта это приведет к катострофе.
...
Зачем это нужно?
++++++++++++++++++
Поэтому рассмотрим применение полиморфизма на примере.
...
Используя полиморфизм, сделайте так, чтобы `__repr__` у них работал по-разному.
