...
Данная работа посвящена обработке с графом статей Википедии и рассчитьана на достижение двух целей:

#. Применение полученных ранее знаний для работы с *реальными* данными.
...
Таким образом, суммарный объём памяти, необходимый для хранения 10 чисел можно быть вычислен следующим образом: размер
списка + 10*(размер целого числа). Для определения объёма памяти, необходимого для хранения объекта, можно
...
К счастью, в стандартную библиотеку языка Python входит класс `array.array`__, который храненит целые числа в памяти в
...
Как можно заметить из примера выше, на архитектуре x86_64 тип с кодом 'I' имеет размер 4 байта, а тип с кодом 'L' — 8
байт. Соответственно, диапазон допустимых значений для первого равен [0; 4294967296), диапазон же второго гораздо шире —
[0; 18446744073709551616).

При этом можно заметить как минимум четырёхкратную экономию памяти. В случае же графа для Википедии можно добиться
...
Теперь перейдём к вопросу о том, каким образом можно представить граф в памяти компьютера без использования списком,
...
Перейдём непосредственно к практике. Цонечная цель работы — получить некоторые статистические данные по графику статей
Википедии, а также построить несколько распределений.
...
Реализуйте метод загрузки графа из текстового файла `WikiGraph.load_from_file`. Для хранения используйте способ,
...
Скачайте полный граф статей Википедии в папку репозитория. Распакуйте его при помощи команды `gzip -dc wiki.txt.gz > wiki.txt`.
Запустите свою программу, передав в качестве входного файла `wiki.txt`.

#. Получите статистические данные и гистограммы, описанные в упражнениях №3 и №4.
#. путь, по которому можно добраться от статьи `Python`__ до статьи `Боль`__.
