...
`<b>..</b>`. Для этого удобно использовать **группы** — нумерованные (или именованные) части шаблона. Для создания
группы нужно просто заключить определённую часть шаблона в круглые скобки `(…)`:
...
при помощи `(?P=имя)` внутри выражения, при помощи `\g<имя>` внутри строки замены  или непосредственно по именни при
...
/ `REGEXP_2_REPL` и т.д., после чего запустите на выполнение тесты из файла `advanced_sub_tests.py`__. Добейтесь
прохождения всех тестов.

.. __: https://github.com/mipt-cs-on-python3/regexp/blob/master/advanced_sub.py
.. __: https://github.com/mipt-cs-on-python3/regexp/blob/master/advanced_sub_tests.py
...
выражения может потебоваться больше времени, чем на решение задачи альтернативным способом.
